...
export async function POST(request: NextRequest) {
  try {
    ...
    // Fetch campaign to get requirements
    const { data: campaign } = await supabase
      .from("campaigns")
      .select("id, status, metadata, description, title")
      .eq("id", campaign_id)
      .maybeSingle();
    ...
    // Get requirements from metadata or description
    ...
    if (!requirements) {
      return NextResponse.json(
        { error: "Campaign has no requirements specified for validation" },
        { status: 400 }
      );
    }
